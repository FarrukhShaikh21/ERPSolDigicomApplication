...
public class CIC_0086 {
    ...
    private RichInputComboboxListOfValues txtStoreNameId;
    ...
    public void doDialogEventSubmit(DialogEvent de) {
        if (de.getOutcome() == DialogEvent.Outcome.yes) {
            ...
            DCIteratorBinding ib = bc.findIteratorBinding("SrvPartsalesCRUDIterator");
            ...
            ob = bc.getOperationBinding("doSubmitPartSale");
            ob.execute();
            ...
            ib = bc.findIteratorBinding("DocGlTransTypePartSaleDETROIterator");
            ib.executeQuery();
    ...
    public String doPrint() {
        ...
        reportBean.setReportServerParam(OracleReport.RS_PARAM_REPORT,"Z:\\\\SERVICE\\\\reports\\\\Partsale.rep");
        reportBean.setReportParameter("USER",DigicomClass.getConnectedUser());
        reportBean.setReportParameter("FORM","srv_partsale");
        reportBean.setReportParameter("COMP","2");
        reportBean.setReportParameter("V_PSID",getIt1().getValue().toString());
    ...
    public List<SelectItem> getSuggestedStore(String resourceName) {
        ...
        vo.setNamedWhereClauseParam("P_ADF_LOCATIONID", (getLocationidId().getValue()==null?"-1":getLocationidId().getValue()));
         ...
         vo.getViewObject().setWhereClause("UPPER(STORE_NAME) LIKE upper('%"+resourceName+"%') AND ROWNUM<=10");
        vo.executeQuery();
    ...
    public void setTxtStoreNameId(RichInputComboboxListOfValues txtStoreNameId) {
        this.txtStoreNameId = txtStoreNameId;
    }

    public RichInputComboboxListOfValues getTxtStoreNameId() {
        return txtStoreNameId;
    }
    ...
    public List<SelectItem> getSuggestedPart(String resourceName) {
        ...
        DCIteratorBinding ib=bc.findIteratorBinding("SrvPartsalesCRUDIterator");
        ApplicationModule am=ib.getViewObject().getApplicationModule();
        ViewObject vo=am.findViewObject("ASBSrvPartSaleDetailPartIdCIC_0086");
        vo.getViewObject().reset();
        vo.setNamedWhereClauseParam("P_ADF_MODEL", (getIt11().getValue()==null?"":getIt11().getValue()));
        vo.setNamedWhereClauseParam("P_ADF_STORE_ID", (getIt2().getValue()==null?"-1":getIt2().getValue()));
        vo.getViewObject().setWhereClause("UPPER(Part_Desc) LIKE upper('%"+resourceName+"%') AND ROWNUM<=100");
        vo.executeQuery();
        while(vo.getViewObject().hasNext()) {
            Row resourceRow=vo.next();
            resourceList.add(new SelectItem(resourceRow.getAttribute("PartDesc"),resourceRow.getAttribute("PartDesc").toString()+"::Part No::"+resourceRow.getAttribute("Partno").toString()+"::Category::"+resourceRow.getAttribute("Category").toString()+"::Stock::"+resourceRow.getAttribute("QtyOnHand").toString()+"::Price::"+resourceRow.getAttribute("Price").toString()));
        }
        return resourceList;
    }
    ...
    public String doNew() {
        ...
        OperationBinding operationBinding = bindings.getOperationBinding("doCreateNewPartSaleRow");
        Object result = operationBinding.execute();
        operationBinding = bindings.getOperationBinding("doCreateNewPartSaleDetRow");
