...
public class CIC_0022 {
    ...
    private RichInputListOfValues poidId;
    ...
    public void setPoidId(RichInputListOfValues poidId) {
        this.poidId = poidId;
    }

    public RichInputListOfValues getPoidId() {
        return poidId;
    }
    ...
    public void doDialogEventSubmit(DialogEvent de) {
        if (de.getOutcome() == DialogEvent.Outcome.yes) {
            ...
            DCIteratorBinding ib = bc.findIteratorBinding("SrvGrnCRUDIterator");
            ...
            ob = bc.getOperationBinding("doSubmitSrvGrn");
            ob.execute();
    ...
    public String doNew() {
        ...
        OperationBinding operationBinding = bindings.getOperationBinding("doCreateNewGRNRow");
        Object result = operationBinding.execute();
        operationBinding = bindings.getOperationBinding("doCreateNewGRNDetRow");
    ...
    public String doPrint() {
        ...
        reportBean.setReportServerParam(OracleReport.RS_PARAM_REPORT,"Z:\\\\SERVICE\\\\reports\\\\SRV_GRN.rep");
        reportBean.setReportParameter("USER",DigicomClass.getConnectedUser());
        reportBean.setReportParameter("FORM","Service GRN");
        reportBean.setReportParameter("COMP","2");
        reportBean.setReportParameter("GRNID",getIt1().getValue().toString());
    ...
    public List<SelectItem> getSuggestedSupplier(String resourceName) {
        ...
        vo.getViewObject().setWhereClause("UPPER(Supp_Name) LIKE upper('%"+resourceName+"%')");
        vo.executeQuery();
        while(vo.getViewObject().hasNext()) {
            Row resourceRow=vo.next();
    ...
    public List<SelectItem> getSuggestedParts(String resourceName) {
        List<SelectItem> resourceList=new ArrayList<SelectItem>();
        DCBindingContainer bc=(DCBindingContainer)BindingContext.getCurrent().getCurrentBindingsEntry();
        DCIteratorBinding ib=bc.findIteratorBinding("SrvGrnCRUDIterator");
        ApplicationModule am=ib.getViewObject().getApplicationModule();
        ViewObject vo=am.findViewObject("ASBSrvGrnPartIdCIC_0022");
        vo.getViewObject().reset();
        
        vo.setNamedWhereClauseParam("P_ADF_POID", (getPoidId().getValue()==null?"-1":getPoidId().getValue()));
