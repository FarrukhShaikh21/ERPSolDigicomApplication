...
public class CIC_0028 {
    ...
    private RichInputListOfValues issueIdId;
    ...
    public void setIssueIdId(RichInputListOfValues issueIdId) {
        this.issueIdId = issueIdId;
    }

    public RichInputListOfValues getIssueIdId() {
        return issueIdId;
    }
    ...
    public void doDialogEventSubmit(DialogEvent de) {
        if (de.getOutcome() == DialogEvent.Outcome.yes) {
            ...
            DCIteratorBinding ib = bc.findIteratorBinding("SrvIssuereturnCRUDIterator");
            ...
            ob = bc.getOperationBinding("doSubmitIssueReturn");
            ob.execute();
    ...
    public String doPrint() {
        ...
        reportBean.setReportServerParam(OracleReport.RS_PARAM_REPORT,"Z:\\\\SERVICE\\\\reports\\\\Issue_Parts_Return.rep");
        reportBean.setReportParameter("USER",DigicomClass.getConnectedUser());
        reportBean.setReportParameter("FORM","Srv_Issue_return");
        reportBean.setReportParameter("COMP","2");
        reportBean.setReportParameter("issurret",getIt1().getValue().toString());
    ...
    public List<SelectItem> getSuggestedStore(String resourceName) {
        ...
        DCIteratorBinding ib=bc.findIteratorBinding("SrvIssuereturnCRUDIterator");
        ApplicationModule am=ib.getViewObject().getApplicationModule();
        ViewObject vo=am.findViewObject("ASBSrvIretDetailStoreIdCIC_0028");
        vo.getViewObject().reset();
        vo.setNamedWhereClauseParam("P_ADF_LOCATIONID", (getLocationidId().getValue()==null?"":getLocationidId().getValue()));
        vo.setNamedWhereClauseParam("P_ADF_ISSUE_STOREID", (getIt8().getValue()==null?"":getIt8().getValue()));
        vo.setNamedWhereClauseParam("P_ADF_REJECT_QTY", (getIt9().getValue()==null?"-1":getIt9().getValue()));
        vo.setNamedWhereClauseParam("P_ADF_ISSUE_STOREID_TYPEID", (getIt4().getValue()==null?"":getIt4().getValue()));
