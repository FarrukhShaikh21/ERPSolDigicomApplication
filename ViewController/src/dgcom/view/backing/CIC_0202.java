...
public class CIC_0202 {
    ...
    public void doDialogEventSubmit(DialogEvent de) {
        if (de.getOutcome() == DialogEvent.Outcome.yes) {
            ...
            DCIteratorBinding ib = bc.findIteratorBinding("SrvStocktransferCRUDIterator");
            ...
            ob = bc.getOperationBinding("doSubmitSrvStockStranfer");
            ob.execute();
    ...
    public List<SelectItem> getSuggestedToPart(String resourceName) {
        List<SelectItem> resourceList=new ArrayList<SelectItem>();
        DCBindingContainer bc=(DCBindingContainer)BindingContext.getCurrent().getCurrentBindingsEntry();
        DCIteratorBinding ib=bc.findIteratorBinding("SrvStocktransferCRUDIterator");
        ApplicationModule am=ib.getViewObject().getApplicationModule();
        ViewObject vo=am.findViewObject("ASBInPartsCIC_0202");
        vo.getViewObject().reset();
    ...
    public List<SelectItem> getSuggestedFromPart(String resourceName) {
        List<SelectItem> resourceList=new ArrayList<SelectItem>();
        DCBindingContainer bc=(DCBindingContainer)BindingContext.getCurrent().getCurrentBindingsEntry();
        DCIteratorBinding ib=bc.findIteratorBinding("SrvStocktransferCRUDIterator");
        ApplicationModule am=ib.getViewObject().getApplicationModule();
        ViewObject vo=am.findViewObject("ASBSrvStockTransferDetailFromPartIDCIC_0202");
        vo.getViewObject().reset();
        vo.setNamedWhereClauseParam("P_ADF_TDATE", ib.getCurrentRow().getAttribute("Tdate"));
        vo.getViewObject().setWhereClause("UPPER(Description) LIKE upper('%"+resourceName+"%')");
        vo.executeQuery();
        while(vo.getViewObject().hasNext()) {
            Row resourceRow=vo.next();
