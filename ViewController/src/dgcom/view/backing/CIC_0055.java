...
public class CIC_0055 {
    ...
    public void doDialogEventSubmit(DialogEvent de) {
        if (de.getOutcome() == DialogEvent.Outcome.yes) {
            ...
            DCIteratorBinding ib = bc.findIteratorBinding("SrvJobreturnCRUDIterator");
            OperationBinding ob = bc.getOperationBinding("Commit");
            System.out.println("Pakistan-1");
            bc.execute();
            System.out.println("Pakistan-2");
            ib.getViewObject().getCurrentRow().setAttribute("Posted", "Y");
            System.out.println("Pakistan-3");
            ob.execute();
            System.out.println("Pakistan-4");
            if (!ob.getErrors().isEmpty()) {
                ib.getCurrentRow().refresh(ib.getViewObject().getCurrentRow().REFRESH_UNDO_CHANGES);
                return;
            }
            System.out.println("Pakistan-5");
            ob = bc.getOperationBinding("doSubmitJobReturn");
            System.out.println("Pakistan-6");
            ob.execute();
            System.out.println("Pakistan-7");
            if (!ob.getErrors().isEmpty()) {
                ib.getCurrentRow().refresh(ib.getViewObject().getCurrentRow().REFRESH_UNDO_CHANGES);
                return;
            }
            System.out.println("Pakistan-8");
    ...
    public String doNew() {
        BindingContainer bindings = getBindings();
        OperationBinding operationBinding = bindings.getOperationBinding("doCreateNewJobReturnRow");
        Object result = operationBinding.execute();
        operationBinding = bindings.getOperationBinding("doCreateNewJobRetDetRow");
    ...
    public String doPrint() {
        ...
        reportBean.setReportServerParam(OracleReport.RS_PARAM_REPORT,"Z:\\\\SERVICE\\\\reports\\\\SRV_JOBRETURN.rep");
        reportBean.setReportParameter("USER",DigicomClass.getConnectedUser());
        reportBean.setReportParameter("FORM","SRV_JOBRETURN");
        reportBean.setReportParameter("COMP","2");
        reportBean.setReportParameter("JOBRETID",getIt1().getValue().toString());
